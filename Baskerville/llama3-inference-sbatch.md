...
Because we’re using `torchrun` we have to create an execution wrapper so that we can pass in the node ranks.
SLURM will set a different value to `${SLURM_NODEID}` for each wrapper running on each node.
We need to pass these different values on to `torchrun`.

You should create this script in the root of your `llama3` repository and call it `wrapper.sh`.
You can either create it directly on Baskerville or create it somewhere else (on your local machine?) and transfer it over.
