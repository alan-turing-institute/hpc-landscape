...
When `torchrun` is used directly in thsi way it doesn’t support Python virtual environments because it defaults to the system-installed Python.
...
## 4. Log in to a single node with a single GPU and a suitable configuration:
...
## 5. Set up the environment on the node:
...
We’re assuming here that the virtual environment was already created in step 2 above, so we just activate it.
If you’ve not already done this then you’ll need to run the commands there to create it as well.

## 6. Run the example script that performs a bunch of inference steps:
...
## 10. Log in to the secondary node.

If you’re using screen or tmux you can switch to a new shell.
...
## 11. Configure the secondary node.
...
## 12. Execute the example script on the primary node:
...
## 13. Execute the example script on the secondary node:

Now move to the terminal on the secondary node and run this similar &mdash; but not quote identical &mdash; command.
